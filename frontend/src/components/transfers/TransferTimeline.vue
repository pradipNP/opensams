...
const steps = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      at: item.approvedAt,
      detail: item.approvedBy?.fullName || '',
    },
    {
      key: 'rejected',
      label: 'Rejected',
    ...
    },
    {
      ...
      at: status === 'cancelled' ? item.updatedAt : null,
      detail: status === 'cancelled' ? item.notes || '' : '',
    },
  ];
});
</script>
...
<template>
  <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
    <h2 class="text-lg font-semibold text-navy-950">Timeline</h2>
