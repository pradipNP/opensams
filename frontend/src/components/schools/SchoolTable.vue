...
<template>
  <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm">
        <thead class="bg-slate-50 text-left text-xs font-semibold tracking-wide text-slate-600 uppercase">
          <tr>
            ...
            <th class="px-4 py-3 whitespace-nowrap">School Code</th>
        ...
        <tbody class="divide-y divide-slate-100 bg-white text-slate-800">
          <tr v-if="loading && !schools.length">
            <td colspan="7" class="px-4 py-10 text-center text-slate-500">Loading schools…</td>
          </tr>
          <tr v-else-if="!schools.length">
            <td colspan="7" class="px-4 py-10 text-center text-slate-500">No schools match the current filters.</td>
          </tr>
          <tr v-for="school in schools" :key="school.id" class="hover:bg-slate-50">
            <td class="px-4 py-3 font-medium text-navy-950">{{ school.name }}</td>
            <td class="px-4 py-3 whitespace-nowrap">{{ school.schoolCode }}</td>
            <td class="px-4 py-3 whitespace-nowrap">{{ school.schoolType }}</td>
            <td class="px-4 py-3">{{ school.municipalityName || '—' }}</td>
            <td class="px-4 py-3 whitespace-nowrap">{{ formatNumber(school.assetCount) }}</td>
            ...
            <td class="px-4 py-3 text-right whitespace-nowrap">
              <RouterLink :to="{ name: 'school-detail', params: { id: school.id } }" class="text-navy-800 hover:underline">
                View
              </RouterLink>
              <RouterLink
                v-if="canWrite"
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3 text-sm text-slate-600">
      <p>{{ loading && schools.length ? 'Refreshing…' : rangeLabel }}</p>
